...
## 現行実装の置き場所（2026-09-05）

このフォルダは実験テキスト・配布物の参照元である。今回のユーザー指定により、実行する正本は [experiments/E0](../E0/README.md) からの段階別フォルダ、共通コードは [experiments/src/core.py](../src/core.py) とする。以下の子src・configs・runs構成やZIP展開手順は配布スターターの説明であり、並行する実装正本を新設する指示ではない。
