...
実験の結果・入力データ・図を探す入口は **[実験結果・データ対応台帳](experiments/RESULTS_INDEX.md)**。現行系列と旧runを区別し、条件表・NPZ・予測・図の元データ・保存不足を整理している。今後の実験では [記録ガイド](experiments/RECORDING_GUIDE.md) に従い、レポートと台帳を同時に更新する。
